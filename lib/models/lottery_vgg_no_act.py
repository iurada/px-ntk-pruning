...
class VGG(nn.Module):
    ...
    def __init__(self, plan, conv, num_classes=10, dense_classifier=False):
        ...
        layer_list = []
        filters = 3
        self.config = ['p1', 'p2', '1.0']
        self.config = ['p1', '1.0', '1.0']
        self.config = ['1.0', 'p2', '1.0']
    ...
    def set_activations(self, state: bool):
        for layer in self.layers:
            if isinstance(layer, (ConvModule, ConvBNModule)):
                layer.relu.active = state
...
def _vgg(arch, plan, conv, num_classes, dense_classifier, pretrained):
    ...
    if pretrained:
        ...
        pretrained_path = f'record/CIFAR100/{arch}/Pretrain/last.pth'
