...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        #if not isinstance(self.shortcut, layers.Identity2d):
        out += self.shortcut(x) #!
        out = self.relu2(out)
...
class ResNet(nn.Module):
    ...
    def set_activations(self, state: bool):
        self.relu.active = state
        for block in self.blocks:
            block.relu1.active = state
            block.relu2.active = state
...
def _resnet(arch, plan, num_classes, dense_classifier, pretrained):
    ...
    if pretrained:
        ...
        pretrained_path = f'record/CIFAR10/{arch}/Pretrain/last.pth'
