...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        return layers.ReLU(inplace=True)(self.residual_function(x) + self.shortcut(x))
...
def resnet34(num_classes, dense_classifier=False, pretrained=False):
...
def resnet101(num_classes, dense_classifier=False, pretrained=False):
    """ return a ResNet 101 object
    """
    return _resnet('resnet101', BottleNeck, [3, 4, 23, 3], 64, num_classes, dense_classifier, pretrained)

def resnet152(num_classes, dense_classifier=False, pretrained=False):
